...
namespace ET
{
    public class EventSystem : Singleton<EventSystem>, ISingletonAwake
    {
        ...
        {
            public IPublish IEvent { get; }

            public int SceneType { get; }

            public EventInfo(IPublish iEvent, int sceneType)
            {
                this.IEvent = iEvent;
        ...
        private readonly Dictionary<Type, List<EventInfo>> allPublishers = new();
        ...
        public void Awake()
        {
            ...
            foreach (Type type in codeTypes.GetTypes(typeof(PublishAttribute)))
            {
                IPublish obj = Activator.CreateInstance(type) as IPublish;
                if (obj == null)
                {
                    throw new Exception($"type not is AEvent: {type.Name}");
                }

                object[] attrs = type.GetCustomAttributes(typeof(PublishAttribute), false);
                foreach (object attr in attrs)
                {
                    PublishAttribute eventAttribute = attr as PublishAttribute;

                    Type eventType = obj.Type;

                    EventInfo eventInfo = new(obj, eventAttribute.SceneType);

                    if (!this.allPublishers.ContainsKey(eventType))
                    {
                        this.allPublishers.Add(eventType, new List<EventInfo>());
                    }
                    this.allPublishers[eventType].Add(eventInfo);
                }
            }
        ...
        public async ETTask PublishAsync<T>(Scene scene, T a) where T : struct
        {
            ...
            if (!this.allPublishers.TryGetValue(typeof(T), out iEvents))
            {
                return;
            }
            ...
            foreach (EventInfo eventInfo in iEvents)
            {
                ...
                if (!(eventInfo.IEvent is APublishHandler<T> aEvent))
                {
